...
import (
	...
	"github.com/denstepanov/algostructs/src/structure/linear/linked-list/singly"
)
...
func TestFindByIndex(t *testing.T) {
	...
	if node != list.Tail() {
		t.Fatal(fmt.Printf("SLList.FindByIndex() %s", util.MethodDoesNotWork))
	}
}
...
func TestFindByValue(t *testing.T) {
	...
	if nodes[0] != list.Tail() {
		t.Fatal(fmt.Printf("SLList.FindByValue() %s", util.MethodDoesNotWork))
	}
}

func TestInsertTail(t *testing.T) {
	if list.Len() == 0 {
		t.Fatal(fmt.Printf("SLList.InsertTail() %s", util.MethodDoesNotWork))
	}
}
...
func TestClear(t *testing.T) {
	...
	if !list.IsEmpty() {
		t.Fatal(fmt.Printf("SLList.Clear() %s", util.MethodDoesNotWork))
	}
...
func TestToSlice(t *testing.T) {
	...
	if len(listAsSlice) != list.Len() {
		t.Fatal(fmt.Printf("SLList.ToSlice() %s", util.MethodDoesNotWork))
	}
}
...
func TestInsertBefore(t *testing.T) {
	...
	if list.Len() == oldLen {
		t.Fatal(fmt.Printf("SLList.InsertBefore() %s", util.MethodDoesNotWork))
	}
}
...
func TestInsertAfter(t *testing.T) {
	...
	if list.Len() == oldLen {
		t.Fatal(fmt.Printf("SLList.InsertAfter() %s", util.MethodDoesNotWork))
	}
}
...
func TestReverse(t *testing.T) {
	...
	if list.Head() != oldTail && list.Tail() != oldHead {
		t.Fatal(fmt.Printf("SLList.Reverse() %s", util.MethodDoesNotWork))
	}
}
