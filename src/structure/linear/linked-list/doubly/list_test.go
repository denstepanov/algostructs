...
import (
	...
	"github.com/denstepanov/algostructs/src/structure/linear/linked-list/doubly"
)
...
func TestFindByIndex(t *testing.T) {
	...
	if node != list.Tail() {
		t.Fatal(fmt.Printf("DLList.FindByIndex() %s", util.MethodDoesNotWork))
	}
}
...
func TestFindByValue(t *testing.T) {
	...
	if nodes[0] != list.Tail() {
		t.Fatal(fmt.Printf("DLList.FindByValue() %s", util.MethodDoesNotWork))
	}
}

func TestInsertTail(t *testing.T) {
	if list.Len() == 0 {
		t.Fatal(fmt.Printf("DLList.InsertTail() %s", util.MethodDoesNotWork))
	}
}
...
func TestClear(t *testing.T) {
	...
	if !list.IsEmpty() {
		t.Fatal(fmt.Printf("DLList.Clear() %s", util.MethodDoesNotWork))
	}
...
func TestToSlice(t *testing.T) {
	...
	if len(listAsSlice) != list.Len() {
		t.Fatal(fmt.Printf("DLList.ToSlice() %s", util.MethodDoesNotWork))
	}
}
...
func TestInsertBefore(t *testing.T) {
	...
	if list.Len() == oldLen {
		t.Fatal(fmt.Printf("DLList.InsertBefore() %s", util.MethodDoesNotWork))
	}
}
...
func TestInsertAfter(t *testing.T) {
	...
	if list.Len() == oldLen {
		t.Fatal(fmt.Printf("DLList.InsertAfter() %s", util.MethodDoesNotWork))
	}
}
